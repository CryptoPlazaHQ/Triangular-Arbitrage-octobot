...
def save_arbitrage_run(exchange, profit_percentage, opportunities):
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.commit()
            logging.info(f"Arbitrage run {run_id} saved successfully.")
    except Exception as e:
        logging.error(f"Error saving arbitrage run: {e}")
...
def get_historical_profit_trend(exchange, limit=100, start_date=None, end_date=None):
    ...
    try:
        ...
        logging.error(f"Error retrieving historical profit trend: {e}")
